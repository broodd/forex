...
import cls from './lessons-table.module.scss'

interface ILessonsTableProps {
  className?: string
}

export const LessonsTable: FC<ILessonsTableProps> = ({ className }) => {
  ...
  return (
    <>
      {lessons.length > 0 ? (
        ...
        <PageEmpty text={t('PAGES.LESSONS.EMPTY')} render={!isLoading} />
      )}
    </>
  )
}
