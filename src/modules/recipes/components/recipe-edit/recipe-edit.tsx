...
import { useEditRecipe } from './hooks'
...
export const RecipeEdit = () => {
  ...
  const [form] = Form.useForm<IEditRecipeFormValues>()
  const { closeDrawer } = useDrawerContext()
  const { onFinish, isLoading, initialValues, onClose } = useEditRecipe(closeDrawer, form)
  const { isUploadLoading } = useUploadStore()
  return (
    <DrawerElement drawerKey={EDrawerRecipesKeys.RECIPE_EDIT}>
      <Form
        clearOnDestroy
        form={form}
        name='edit_recipe'
      ...
      >
        <DrawerCard>
          <DrawerHeader title={t('PAGES.RECIPES.EDIT_RECIPE')} onClose={onClose} />
          <DrawerBody>
